...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$src = '%~dp0'.TrimEnd('\'); ^
     ...
     $exclude = @('.venv','__pycache__','voice_typer.log','_setup_test.log','GigaAM-Voice-Portable.zip'); ^
     Get-ChildItem $src -Force | Where-Object { $exclude -notcontains $_.Name } | ForEach-Object { Copy-Item $_.FullName -Destination $tmp -Recurse -Force }; ^
